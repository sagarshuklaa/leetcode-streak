...
public class Solution {
    ...
    public int[] assignEdgeWeights(int[][] edges, int[][] queries) {
        ...
        for (int i = 0; i < queries.length; ++i) {
            final int u = queries[i][0], v = queries[i][1];
    ...
    private void dfs(int u, int p, List<Integer>[] graph, int[][] parent, int[] depth) {
        parent[0][u] = p;
        for (int v : graph[u]) if (v != p) { depth[v] = depth[u] + 1; dfs(v, u, graph, parent, depth); }
    }
    ...
    private int lca(int u, int v, int[][] parent, int[] depth) {
        ...
        if (u == v) return u;
        for (int k = LOG - 1; k >= 0; --k)
            if (parent[k][u] != -1 && parent[k][u] != parent[k][v]) { u = parent[k][u]; v = parent[k][v]; }
        return parent[0][u];
    }

    private int modPow(long x, long n) {
        if (n == 0) return 1;
        if (n % 2 == 1) return (int) (x * modPow(x % MOD, n - 1) % MOD);
