// Problem: Maximum Building Height
// Link:    https://leetcode.com/problems/maximum-building-height/
// Date:    2026-06-20
// Time:    O(k log k)
// Space:   O(k)

import java.util.*;

public class Solution {
    public int maxBuilding(int n, int[][] restrictions) {
        final int k = restrictions.length;
        int[][] A = new int[k + 2][2];
        System.arraycopy(restrictions, 0, A, 0, k);
        A[k] = new int[] {1, 0};
        A[k + 1] = new int[] {n, n - 1};

        Arrays.sort(A, Comparator.comparingInt((int[] a) -> a[0]).thenComparingInt(a -> a[1]));

        // Forward pass: tighten constraints based on distance from previous building
        for (int i = 1; i < A.length; ++i) {
            final int dist = A[i][0] - A[i - 1][0];
            A[i][1] = Math.min(A[i][1], A[i - 1][1] + dist);
        }

        // Backward pass: tighten constraints based on distance from next building
        for (int i = A.length - 2; i >= 0; --i) {
            final int dist = A[i + 1][0] - A[i][0];
            A[i][1] = Math.min(A[i][1], A[i + 1][1] + dist);
        }

        int ans = 0;

        // For each pair of consecutive constrained buildings, find the peak between them
        for (int i = 1; i < A.length; ++i) {
            final int l = A[i - 1][0];
            final int r = A[i][0];
            final int hL = A[i - 1][1];
            final int hR = A[i][1];
            ans = Math.max(ans, Math.max(hL, hR) + (r - l - Math.abs(hL - hR)) / 2);
        }

        return ans;
    }
}
